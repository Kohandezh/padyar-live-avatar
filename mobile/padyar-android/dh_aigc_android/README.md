# Padyar Mobile for Android SDK Documentation

English | [中文](./README_zh.md)

## 1. Product Overview

`Padyar Mobile for Android` is a lightweight, fully offline 2D digital human solution for Android, supporting real-time rendering of digital avatars driven by voice audio.
...
| PADYAR               | Main control object of the digital human, integrates model loading, rendering, broadcasting, and motion control |
...
1. Obtain the complete source package, unzip it, and copy the `padyar-sdk` directory to the project root directory.
...
```gradle
dependencies {
    api project(":padyar-sdk")
}
```

### 3.2 AAR Reference (Optional)

1. Place the compiled `padyar-sdk-release.aar` module into the `libs/` directory.
...
```mermaid
graph TD
A[Check Configuration and Models] --> B[Build PADYAR Instance]
...
> Function Definition: `ai.guiji.padyar.sdk.client.VirtualModelUtil`
...
In the `onCreate()` stage of the rendering page, build the PADYAR object and call the init interface.

> Function Definition: `ai.guiji.padyar.sdk.client.PADYAR`

```
// Build PADYAR object
public PADYAR(Context context, String modelName, RenderSink sink, Callback callback)

// Initialize PADYAR service
void init()
```

**PADYAR Object Construction Explanation**:
...
Where **Callback** is defined as: `ai.guiji.padyar.sdk.client.Callback`
...
```kotlin
padyar = PADYAR(mContext, modelUrl, mPADYARRender) { event, msg, info ->
    when (event) {
        ai.guiji.padyar.sdk.client.Constant.CALLBACK_EVENT_INIT_READY -> {
            initOK()
        }

        ai.guiji.padyar.sdk.client.Constant.CALLBACK_EVENT_INIT_ERROR -> {
            initError()
        }
        // ...
    }
...
Use the SDK-provided `PADYARRenderer` and `PADYARTextureView` to quickly implement rendering with transparency support. Alternatively, you can implement the `RenderSink` interface to customize the rendering logic.

The **RenderSink** definition is as follows: `ai.guiji.padyar.sdk.client.render.RenderSink`
...
Use `PADYARRenderer` and `PADYARTextureView` to quickly implement rendering. These controls support transparency and can freely set the background and foreground.
...
override fun onCreate(savedInstanceState: Bundle?) {
    ...
    binding.glTextureView.setRenderer(mPADYARRender)
    binding.glTextureView.renderMode =
        GLSurfaceView.RENDERMODE_WHEN_DIRTY      // Must be called after setting the renderer

    padyar = PADYAR(mContext, modelUrl, mPADYARRender) { event, msg, _ ->
    }
    // ...
}
```
...
```kotlin
padyar?.startMotion("Greeting", true)
```
...
```proguard
-keep class ai.guiji.padyar.PadyarNcnn{*; }
```
...
5. Insufficient device performance may result in the audio feature extraction speed not matching the playback speed. You can use `padyar?.setReporter()` to monitor frame rendering information.
