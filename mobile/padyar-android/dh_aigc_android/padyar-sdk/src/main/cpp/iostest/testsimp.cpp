...
static void* mfccworker(void* arg){
  ...
  for(int k=0;k<1;k++){
    ...
    sessid = dhpadyar_newsession(mfcc);
    g_sessid = sessid;
    int psize = 1000;
    int tickcnt = 0;
    int kkk = 0;
    ...
    while(sessid == g_sessid){
      ...
      if(readpcm<1)break;
      dhpadyar_pushpcm(mfcc,sessid,pcm,readpcm,0);
      tickcnt += readpcm;
    ...
    dhpadyar_finsession(mfcc,sessid);
    free(pcm);
    fclose(g_wavfile);
    printf("===finish\n");
  }
...
int mainmemcheck(int argc,char** argv){
  ...
  int width = 540;
  int height = 720;
  rst = dhpadyar_alloc(&dg,20,width,height);
  //char* fnwenet = "model/wenet.onnx";
  rst = dhpadyar_initPcmex(dg,0,10,20,50,0);
  rst = dhpadyar_initMunetex(dg,"model/xinyan_opt.param","model/xinyan_opt.bin","model/weight_168u.bin",128);
  //char* fnwenet = "model/wenet.onnx";
  char* fnwenet = "model/wenet.onnx";
  rst = dhpadyar_initWenet(dg,fnwenet);
  ...
  int* box = m_boxs;
  for(int m=0;m<10;m++){
    g_sessid = dhpadyar_newsession(dg);
    for(int k=0;k<100;k++){
      dhpadyar_pushpcm(dg,g_sessid,pcm,102400,0);
      int allcnt = dhpadyar_allcnt(dg,g_sessid);
      printf("===allcnt %d\n",allcnt);
    }
    int readycnt = dhpadyar_readycnt(dg,g_sessid);
    while(readycnt<1){
      jtimer_mssleep(10);
    }
    for(int i=0;i<100;i++){
      readycnt = dhpadyar_readycnt(dg,g_sessid);
      //printf("===readycnt %d\n",readycnt);
      rst = dhpadyar_simpinx(dg,g_sessid, bpic,width,height, box, bmsk, bfg, i);
      printf("==simp %d\n",rst);
      jtimer_mssleep(10);
      if(rst<0)break;
    }
    dhpadyar_finsession(dg,g_sessid);
  }
  free(pcm);
  delete mat_pic;
  delete mat_msk;
...
int main(int argc,char** argv){
  ...
  while(1){
    ...
    rst = 1;//dhpadyar_readpcm(dg,g_sessid,pcm,pcmsize,bnf,bnfsize);
    printf("===readpcm %ld %d\n",g_sessid,rst);
    if(rst>0){
      uint64_t tick = jtimer_msstamp();
      //printf("====read  %ld \n",tick);
      //rst = dhpadyar_simprst(dg,g_sessid, bpic,width,height, box, bmsk, bfg, (uint8_t*)bnf,bnfsize);
      rst = dhpadyar_simpinx(dg,g_sessid, bpic,width,height, box, bmsk, bfg, bnfinx);
      if(rst>0)bnfinx ++;
      printf("===simprst %d\n",rst);
      mat_fg->show("aaa");
      cv::waitKey(20);
  ...
  printf("====exit\n");
  //
  rst = dhpadyar_free(dg);
  printf("====exitmfcc\n");
  /*
     rst = dhpadyar_free(unet);
     delete mat_pic;
     delete mat_msk;
     delete mat_fg;
     */
