...
static void *calcworker(void *arg){
  dhpadyar_t* mfcc = (dhpadyar_t*)arg;
  uint64_t sessid = 0;
  while(mfcc->running){
    int rst = 0;
...
int dhpadyar_alloc(dhpadyar_t** pdg,int mincalc,int width,int height){
  dhpadyar_t* padyar = (dhpadyar_t*)malloc(sizeof(dhpadyar_t));
  memset(padyar,0,sizeof(dhpadyar_t));
  padyar->mincalc = mincalc?mincalc:1;
  padyar->minoff = STREAM_BASE_MINOFF;
  padyar->minblock = STREAM_BASE_MINBLOCK;
  padyar->maxblock = STREAM_BASE_MAXBLOCK;
  pthread_mutex_init(&padyar->pushmutex,NULL);
  pthread_mutex_init(&padyar->readmutex,NULL);
  pthread_mutex_init(&padyar->freemutex,NULL);
  padyar->slist = new std::queue<PcmSession*>();
  padyar->calcthread = (pthread_t *)malloc(sizeof(pthread_t) );
  padyar->running = 1;
  pthread_create(padyar->calcthread, NULL, calcworker, (void*)padyar);
  padyar->rgb = 1;
  padyar->width = width;
  padyar->height = height;
  padyar->mat_msk = new JMat(width,height);
  padyar->mat_fg = new JMat(width,height);
  padyar->mat_pic = new JMat(width,height);
  //padyar->mat_feat = jmat_alloc(20,STREAM_BASE_BNF,1,0,4,NULL);
  padyar->mat_feat = jmat_alloc(STREAM_BASE_BNF,20,1,0,4,NULL);
  padyar->kind = 168;
  padyar->rect = 160;
  *pdg = padyar;
  return 0;
...
int dhpadyar_fileinx(dhpadyar_t* dg,uint64_t sessid,char* fnpic,int* box,char* fnmsk,char* fnfg,int bnfinx,char* bimg,char* mskbuf,int imgsize){
  ...
  int rst = 0;
  if(box){
    rst = dhpadyar_simpinx(dg,sessid, bpic,dg->width,dg->height, box, bmsk, bfg,bnfinx);
  }else{
    rst = dhpadyar_simpblend(dg,sessid, bpic,dg->width,dg->height,  bmsk, bfg);
  }
...
int dhpadyar_simpinx(dhpadyar_t* dg,uint64_t sessid,uint8_t* bpic,int width,int height,int* box,uint8_t* bmsk,uint8_t* bfg,int inx){
  if(sessid!=dg->sessid)return -1;
  if(!dg->running)return -2;
  PcmSession* sess = dg->cursess;
  if(!sess)return -3;
  ...
  //printf("===readblock %d\n",rst);
  if(rst>0){
    rst = dhpadyar_simprst(dg,sessid, bpic,w,h, box, bmsk, bfg,(uint8_t*)dg->mat_feat->data,STREAM_ALL_BNF);
    return 1;
  }
  return rst;
}
