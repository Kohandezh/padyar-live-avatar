...
public class PADYAR {
    ...
    public void init() {
        // 先检查模型文件
        File padyarDir = mContext.getExternalFilesDir("padyar");

        File baseConfigDir = new File(padyarDir + "/model/gj_dh_res");
        File baseConfigTag = new File(padyarDir + "/model/tmp/gj_dh_res");
        ...
        File modelDir = new File(padyarDir + "/model", dirName);
        File modelTag = new File(padyarDir + "/model/tmp", dirName);
        ...
        mRenderThread.setName("PADYARRender-Thread");
        mRenderThread.start();
    }
