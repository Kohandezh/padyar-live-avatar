...
import ai.guiji.padyar.sdk.client.net.DownloadZipService;


public class VirtualModelUtil {

    public static boolean checkBaseConfig(Context context){
        String padyarDir = context.getExternalFilesDir("padyar").getAbsolutePath();
        File baseDir = new File(padyarDir + "/model", "gj_dh_res");
        File baseTag = new File(padyarDir + "/model/tmp", "gj_dh_res");
    ...
    public static boolean checkModel(Context context, String name){
        if (!TextUtils.isEmpty(name)){
            ...
            if (name.startsWith("https://") || name.startsWith("http://")){
                ...
                File modelDir = new File(padyarDir + "/model", name);
                File modelTag = new File(padyarDir + "/model/tmp", name);
                return modelDir.exists() && modelTag.exists();
            }
        } else {
            return false;
