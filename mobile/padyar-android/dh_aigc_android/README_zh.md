# Padyar Mobile for Android SDK 文档

中文 | [English](./README.md)

## 一、产品介绍

`Padyar Mobile for Android` 是一套轻量级、纯离线的 Android 平台 2D 虚拟人解决方案，支持通过语音音频驱动数字人形象并进行实时渲染。
...
| PADYAR              | 数字人主控对象，集成了模型加载、渲染、播报、动作等能力                                            |
...
1. 获取完整源码包，解压后将 `padyar-sdk` 目录复制到项目根目录下。
...
1. 将padyar-sdk模块编译的 `padyar-sdk-release.aar` 放入 `libs/` 目录。
2. 添加依赖：
...
> 函数定义: `ai.guiji.padyar.sdk.client.VirtualModelUtil`
...
在渲染页onCreate()阶段构建PADYAR对象并调用init接口

> 函数定义: `ai.guiji.padyar.sdk.client.PADYAR`
...
其中**Callback**的定义: `ai.guiji.padyar.sdk.client.Callback`
...
使用 SDK 提供的 `PADYARRenderer` 和 `PADYARTextureView` 可快速实现支持透明通道的渲染。也可以自己实现RenderSink接口自定义渲染逻辑。

其中**RenderSink**的定义如下: `ai.guiji.padyar.sdk.client.render.RenderSink`
...
使用PADYARRenderer及PADYARTextureView控件简单实现渲染展示,该控件支持透明通道可以自由设置背景及前景
...
5. 设备性能不足时可能导致音频特征提取的速度跟不上音频播放的速度，可以使用padyar?.setReporter()函数添加一个监控观察帧渲染返回的信息。
