...
NSString * const kUUIDKey = @"com.guiji.padyarSDK";
...
+ (NSString*) value {
    ...
    NSString *getUDIDInKeychain = (NSString *)[GJLOpenUDID load:kUUIDKey];
    if (!getUDIDInKeychain || [getUDIDInKeychain isEqualToString:@""] || [getUDIDInKeychain isKindOfClass:[NSNull class]]) {
        return [GJLOpenUDID valueWithError:nil];
    } else {
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [GJLOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [GJLOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [GJLOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
    ...
    [GJLOpenUDID save:kUUIDKey data:openUDID];
    return kOpenUDIDSessionCache;
}
